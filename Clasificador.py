...
#Gómez Olvera Jacob Misael      16590474
	...
	tweet => Assassin's,Creed,Syndicate,Historia, Video 
...
with open("fedelobo.json", "r") as read_file:
...
def stream(t, CON):
    ...
    probabilidad = []
    if l <= 2:
        return "No se reconoce el Tweet"
    else:
        for i in range(l):
            for j in range (k):
                if CON[j][0] == t[i]:
                    probabilidad.append(CON[j])
        for l in range(len(probabilidad)):
            suma += float(probabilidad[l][1])
            promedio = suma / len(probabilidad)
        if (promedio > .55):
            return "Stream"
        else:
            return "No Hay Stream"

print(stream(tweet,Conocimiento))